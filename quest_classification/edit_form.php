    ...
    protected function specific_definition($mform) {
      global $DB;
        ...
        if (!$this->block->get_owning_quest()) 
        {
            ...
            if(empty($quests)) 
            {
                $mform->addElement('static', 'no_quests_in_course', get_string('error_emptyquestid', 'block_quest_classification'),
                ...
                $mform->addElement('select', 'config_questid', get_string('config_select_quest', 'block_quest_classification'), $quests);
            }
        }

        $mform->addElement('text', 'config_showbest', get_string('config_show_best', 'block_quest_classification'), array('size' => 3));
